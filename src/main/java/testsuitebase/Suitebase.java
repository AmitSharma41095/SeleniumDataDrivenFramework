...
public class Suitebase {
	...
	public void report()
	{
		 htmlReporter = new ExtentHtmlReporter(System.getProperty("user.dir")+"/TestReport/createmodel.html");
		 htmlReporter.config().setTestViewChartLocation(ChartLocation.BOTTOM);
			htmlReporter.config().setChartVisibilityOnOpen(true);
			htmlReporter.config().setTheme(Theme.DARK);

			htmlReporter.config().setEncoding("utf-8");
			htmlReporter.config().setReportName("GRC_Automation_Report");
			htmlReporter.config().setDocumentTitle("GRC_Automation_Report");

			
			extent = new ExtentReports();
			extent.attachReporter(htmlReporter);
			extent.setSystemInfo("Browser", "Chrome v1");
			extent.setSystemInfo("OS", "Windows");
			extent.setSystemInfo("Environment", "SIT");
			extent.setSystemInfo("Team", "Pune");
	}
}
